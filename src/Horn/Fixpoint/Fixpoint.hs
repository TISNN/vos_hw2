...
pred_abs phi preds =  do
        ...
        let pred_abs_new [] = do
                return []
            pred_abs_new (x:xs) = do
                predRest <- pred_abs_new xs
                can_Imply <- implies phi x
                if can_Imply == True
                    then
                        return $ predRest ++ [x]
                    else
...
fixpoint_step preds sol w h = do
        ...
        can_Imply <- implies new_base sol_hd_
...
initWL preds hs vs = do
  ...
  where
    hs' = filter isBase hs
    combine (sol, ws) (sol', ws1) = (Map.union sol sol', Set.union ws ws1)
