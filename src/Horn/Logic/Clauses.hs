...
instance Show (Bound) where
  show b = (show $ bound b) ++ " :- " ++ bd
    where 
      bd = intercalate "∧" (show (bbase b) : (map show $ queries b))
...
-------------------------------------
normalizeVar nv = do
    nmlv <- liftM(\v -> (v, Tr)) nxVar
    let pair = (fst nmlv, (Eq (fst nmlv) nv))
    pure pair
...
-----------------------------------------------
--normalizeQuery nq = do
--    nml_var <- traverse normalizeVar (vars nq)
--    pure (nq { vars = map fst nml_var}, map snd nml_var)  

normalizeQuery (Query n vs) = do
    ret <- mapM(normalizeVar) vs
    let uret = unzip ret
        vs_nml = fst (uret)
        bases = filter(/=Tr) (snd(uret))
        q_nml = Query n vs_nml
    return (q_nml, bases)
...
-----------------------------------------------
normalizeHorn nh = do
    (hd_new, ret) <- normalizeQuery (hd nh)
    bd_new <- traverse normalizeQuery (bd nh)
    return (nh {hd = hd_new, bd = map fst bd_new, base = And (base nh : ret ++ concatMap snd bd_new)}) 
...
normalizeBoundM b = do
    queries' <- traverse normalizeQuery (queries b)
    pure (b { queries = map fst queries', bbase = And (bbase b : concatMap snd queries')}) 
